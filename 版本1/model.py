...
class EHR_MBT_Model(nn.Module):
    def __init__(self, args):
        ...
        # 实验室值嵌入层 - 新增
        #self.lab_code_embedding = nn.Embedding(12332, self.model_dim)
    ...
    def forward(self, batch, missing=None):
        ...
        # 获取各种嵌入
        code_emb = self.code_embedding(medical_codes)  # 代码嵌入
        time_emb = self.time_embedding(medical_times)  # 时间嵌入
        type_emb = self.type_embedding(medical_types)  # 类型嵌入
