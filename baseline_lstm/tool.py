...
def get_history_codes(visits, current_time, index_set):
    """
    按visit分组收集历史codes，只包括current_time所在visit之前的所有完整visit
    """
    visit_codes = []
    visit_times = []
    ...
    for visit in visits:
        ...
        current_visit_codes = [code_idx for code_idx, time in visit
                              if code_idx in index_set and not pd.isna(time)]
        current_visit_times = [time for _, time in visit if not pd.isna(time)]
