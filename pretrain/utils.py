...
def convert_to_visit_level(patient_data, verbose=True):
    """
    将以病人为单位的数据转换为以visit为单位的数据
    ...
    返回:
    visit_data: 以visit为单位的数据列表，每个元素是一个visit字典
    """
    if verbose:
        print(f"开始将病人数据转换为visit数据，原始病人数量: {len(patient_data)}")
    ...
    for patient in patient_data:
        ...
        for visit_idx, visit in enumerate(patient['visits']):
            ...
            visit_record = {
                ...
                'visit': visit,
                'visit_idx': visit_idx  # 可选：记录这是患者的第几次就诊
            }
    ...
    if verbose:
        ...
        print(f"平均每个患者的visit数量: {total_visits / len(patient_data):.2f}")
...
def focal_loss(logits, targets, gamma=2.0, reduction='mean'):
    ...
    Args:
        ...
        targets: [batch_size, vocab_size] - one-hot 编码的真实标签
        gamma: 聚焦参数，默认 2.0
        reduction: 'mean' 或 'sum'，损失的归约方式
